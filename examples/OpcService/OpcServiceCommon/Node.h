...
// MULTILINE_BR is essentially the same as MULTILINE, but the difference is that newlines in the content of the node are
// formatted to use <br> tags.
...
public:
            ...
            if (m_closeTag) {
                ...
                for (std::size_t i = 0; i < m_children.size(); ++i) {
                    ...
                    elem += childNode.ToString(readability, indentLevel + 1) +
                            ((i != m_children.size() - 1 && isMultiline) ? "\n" : "");
                }
    ...
    Node& SetName(const std::string& name)
    {
        ...
        if (periodIndex != std::string::npos || poundIndex != std::string::npos) {
            ...
            const auto ind =
                ((poundBefore || (periodIndex == std::string::npos && poundIndex != std::string::npos)) ? poundIndex
                                                                                                        : periodIndex);
...
private:
    ...
    std::string _ReplaceAllOccurrences(std::string replacer,
